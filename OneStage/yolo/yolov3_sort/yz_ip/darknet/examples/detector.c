...
{ 
    static char name[20]={""};
    char *q = strrchr(p,'/') + 1;
    strncpy(name,q,8);//注意后面的6，如果你的测试集的图片的名字字符（不包括后缀）是其他长度，请改为你需要的长度（官方的默认的长度是6）
    return name;
}
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        //resize_network(net, sized.w, sized.h);
            ...
            int m = plist->size;
            if(access("/DATA/zhanghui/darknet/data/out",0)==-1)//"/home/FENGsl/darknet/data"修改成自己的路径
            {
              if (mkdir("/DATA/zhanghui/darknet/data/out",0777))//"/home/FENGsl/darknet/data"修改成自己的路径
               {
        ...
        if(outfile){
            save_image(im, outfile);
             ...
             char b[2048];
            sprintf(b,"/DATA/zhanghui/darknet/data/out/%s",GetFilename(path));//"/home/FENGsl/darknet/data"修改成自己的路径
